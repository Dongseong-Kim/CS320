  ...
  (cond
    ...
    [else (local [(define a (first arg))]
            (cond
              ...
              [(throw? a) a]
              [else (local [(define check (interp2 (first arg) ds identity exception-k))]
                      (if (throw? check)
                          (throw)
                     ...
                     [closureV (params body ds1)
                               (if (= (length params) (length arg-list))
                                   (local [(define result (dfrsub-for-function params arg-list exception-k ds ds1))]
                                     (cond
                                       [(DefrdSub? result) (interp body result k exception-k)]
                                       [(throw? result) (interp result ds k exception-k)]
                                       [else result]))
                                   (error 'interp "wrong arity: ~a ~a" params arg-list))]
                     [contV (newk)
...
;; interp2 : KCFAE DefrdSub (KCFAE-Value -> alpha) (KCFAE-Value -> alpha) -> alpha-or-(throw)
;; What is different from interp function is that the evaluation of throw is throw itself
;; Also, if the argument of a continuation is throw then the evaluation of the continuation is also throw
;; This function is only for dfrsub-for-function function
(define (interp2 a-fae ds k exception-k)
  (type-case KCFAE a-fae
    [num (n) (k (numV n))]
    [add (l r) (interp2 l ds
                       (lambda (v1)
                         (if (throw? v1)
                             (throw)
                             (interp2 r ds
                       ...
                       (type-case KCFAE-Value fun-val
                         [closureV (params body ds1)
                                   (if (= (length params) (length arg-list))
                                       (local [(define result (dfrsub-for-function params arg-list exception-k ds ds1))]
                                         (cond
                                           [(DefrdSub? result) (local [(define check (interp2 body result k exception-k))]
                                                                 (if (throw? check)
                                                                     (throw)
                                                                     check))]
                                           [else result]))
                                       (error 'interp2 "wrong arity: ~a ~a" (length params) (length arg-list)))]
                         [contV (newk)
                                (if (= 1 (length arg-list))
                                    (interp2 (first arg-list) ds (lambda (val) (newk val)) exception-k)
                                    (error 'interp2 "continuation : only single argument"))]
                         [else (error 'interp2 "not a function")])))
                 exception-k)]
    [if0 (test-expr then-expr else-expr)
         (interp2 test-expr ds
                 (lambda (v)
                   (if (throw? v)
                       (throw)
                       (if (numzero? v)
                           (interp2 then-expr ds k exception-k)
                           (interp2 else-expr ds k exception-k))))
                 exception-k)]
    [withcc (id body-expr)
            (interp2 body-expr 
                    (aSub id
                          ...
                          ds)
                    k exception-k)]
    [try-catch (try-expr catch-expr) (local [(define check (interp2 try-expr ds k (lambda () (interp2 catch-expr ds k exception-k))))]
                                       (if (throw? check)
                                           (interp2 catch-expr ds k exception-k)
                                           check))]
    [throw () (throw)]))
...
;; ----------------------------------------------------------------------------------------------
;; test: interp2---------------------------------------------------------------------------------
(test (interp2 (throw) (mtSub) identity identity) (throw))
(test (interp2 (app (fun (list (id 'x)) (id 'x)) (list (app (fun empty (app (fun empty (throw)) empty)) empty))) (mtSub) identity identity) (throw))
(test (interp2 (app (fun (list (id 'x)) (try-catch (try-catch (num 1) (id 'x)) (num 2))) (list (app (fun empty (app (fun empty (throw)) empty)) empty))) (mtSub) identity identity) (throw))
;; ----------------------------------------------------------------------------------------------
...
(test (run "{{fun {x} {try {{x} 2} catch 42}} {withcc foo {{fun {x} {foo {fun {} {throw}}}} 2}}}") 42)